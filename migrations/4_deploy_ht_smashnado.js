...
module.exports = function(deployer, network, accounts) {
  return deployer.then(async () => {
    const { MERKLE_TREE_HEIGHT, HT_AMOUNT_O } = process.env
    const verifier = await Verifier.deployed()
    const hasherInstance = await hasherContract.deployed()
    await HTSmashnado.link(hasherContract, hasherInstance.address)
    const smashnado = await deployer.deploy(HTSmashnado, verifier.address, HT_AMOUNT_O, MERKLE_TREE_HEIGHT, accounts[0])
