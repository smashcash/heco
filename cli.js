...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 128) {
        console.log(`Transaction submitted through the relay. View transaction on huobi sacan https://hecoinfo.com/`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 128) {
          console.log(`View transaction on huobiscan https://hecoinfo.com/`)
        } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'ht', amount = '1' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/HTSmashnado.json')).json()
  ...
  if (!(netId === 128)) {
    throw new Error('You did not connect to HECO network. Please make sure you connected to HECO Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to HECO Mainnet.')
  erc20 = currency == 'ht' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'ht', amount: '1' }
  //await init(instance)
  if(window.ethereum) {
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient HT Balance in your wallet to deposit.', 'error')
      return false;
    }
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account HT balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
